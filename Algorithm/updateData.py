...
"""We use a loop of calls to Quandl API to get data from each element of our
list of stocks. We take a little over a month of price and volume data.
We use Quandle because it is a reliable and free source of data, though we have to give some time between calls to the 
API to prevent exceeding our call limit for the free account."""
for i in stock_ticker[27:len(stock_ticker)]:
	print (i) #Company being added to database
	url = 'https://www.quandl.com/api/v3/datasets/WIKI/{0}/data.csv?start_date=2015-10-30?api_key=API_KEY_HERE'.format(i)

	#Save csv file to grab data from
	urllib.request.urlretrieve(url, 'test.csv')

	#Create the table in the database
	c.execute("CREATE TABLE '%s' (Date TEXT Primary Key, Open INT, High INT, Low INT, Close INT, Volume INT, Ex_Dividend INT, Split_Ratio INT, Adj_Open INT, Adj_High INT, Adj_Low INT, Adj_Close INT, Adj_Volume INT);"%i)
	connection.commit()
	...
	with open('test.csv', 'r') as f:
		...
		myList = list(reader)
		for row in myList:
			c.executemany("INSERT INTO {} VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?);".format(i), (row,))
			connection.commit()
	f.closed 
